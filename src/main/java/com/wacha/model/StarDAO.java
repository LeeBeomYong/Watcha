...
public class StarDAO {
	...
	// DB와 연동하는 객체.
			...
			// 메인 페이지 리스트 불러오기 (코멘트 TOP10)
				...
				public List<StarDTO> getMainMovie_hit() {
					...
					try {
						...
						sql = "select distinct m.movie_num, m.movie_title, i.image_loc, round(avg(s.movie_star),1) avg, sum(s.movie_watch) now from movie m, image i, star s where m.movie_num = i.movie_num and i.movie_num = s.movie_num group by m.movie_title, i.image_loc, m.movie_num order by now desc, movie_num";
		...
		// 장르 - 별점 10
		public List<StarDTO> selectGenre_star(String genre) {
			List<StarDTO> list = new ArrayList<StarDTO>();
		...
		// 장르 - 보관함10
		public List<StarDTO> selectGenre_heart(String genre) {
			List<StarDTO> list = new ArrayList<StarDTO>();
		...
		// 장르 - 실시간10
				public List<StarDTO> selectGenre_now(String genre) {
					List<StarDTO> list = new ArrayList<StarDTO>();
